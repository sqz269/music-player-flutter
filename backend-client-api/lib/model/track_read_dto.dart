...
class TrackReadDto {
  /// Returns a new [TrackReadDto] instance.
  TrackReadDto({
    this.id,
    this.name,
    this.index,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is TrackReadDto &&
     other.id == id &&
     other.name == name &&
     other.index == index &&
     other.disc == disc &&
  ...
  @override
  String toString() => 'TrackReadDto[id=$id, name=$name, index=$index, disc=$disc, duration=$duration, genre=$genre, staff=$staff, arrangement=$arrangement, vocalist=$vocalist, lyricist=$lyricist, original=$original, originalNonTouhou=$originalNonTouhou, album=$album, trackFile=$trackFile]';
  ...
  /// Returns a new [TrackReadDto] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static TrackReadDto? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "TrackReadDto[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "TrackReadDto[$key]" has a null value in JSON.');
        });
        return true;
      }());
      ...
      return TrackReadDto(
        ...
        album: AlbumReadDto.fromJson(json[r'album']),
  ...
  static List<TrackReadDto> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <TrackReadDto>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = TrackReadDto.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, TrackReadDto> mapFromJson(dynamic json) {
    final map = <String, TrackReadDto>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = TrackReadDto.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of TrackReadDto-objects as value to a dart map
  static Map<String, List<TrackReadDto>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<TrackReadDto>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = TrackReadDto.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
