...
class AlbumReadDto1 {
  /// Returns a new [AlbumReadDto1] instance.
  AlbumReadDto1({
    this.id,
    this.albumName,
    this.releaseDate,
  ...
  List<CircleReadDto1>? albumArtist;

  List<String>? dataSource;

  List<TrackReadDto1>? tracks;
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is AlbumReadDto1 &&
     other.id == id &&
  ...
  @override
  String toString() => 'AlbumReadDto1[id=$id, albumName=$albumName, releaseDate=$releaseDate, releaseConvention=$releaseConvention, catalogNumber=$catalogNumber, numberOfDiscs=$numberOfDiscs, website=$website, albumArtist=$albumArtist, dataSource=$dataSource, tracks=$tracks, thumbnail=$thumbnail, otherFiles=$otherFiles]';
  ...
  /// Returns a new [AlbumReadDto1] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static AlbumReadDto1? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "AlbumReadDto1[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "AlbumReadDto1[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return AlbumReadDto1(
        id: mapValueOfType<String>(json, r'id'),
        albumName: LocalizedField1.fromJson(json[r'albumName']),
        ...
        website: json[r'website'] is List
            ...
            : const [],
        albumArtist: CircleReadDto1.listFromJson(json[r'albumArtist']),
        dataSource: json[r'dataSource'] is List
            ? (json[r'dataSource'] as List).cast<String>()
            : const [],
        tracks: TrackReadDto1.listFromJson(json[r'tracks']),
  ...
  static List<AlbumReadDto1> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <AlbumReadDto1>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = AlbumReadDto1.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, AlbumReadDto1> mapFromJson(dynamic json) {
    final map = <String, AlbumReadDto1>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = AlbumReadDto1.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of AlbumReadDto1-objects as value to a dart map
  static Map<String, List<AlbumReadDto1>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<AlbumReadDto1>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = AlbumReadDto1.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
